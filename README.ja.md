...
<h1>Mimiclaw: Go で書かれた超効率 AI アシスタント</h1>
...
## 📢 ニュース
2026-02-09 🎉 Mimiclaw リリース！$10 ハードウェアで 10MB 未満の RAM で動く AI エージェントを 1 日で構築。🦐 行くぜ、シャコ！
...
### 🐜 革新的な省フットプリントデプロイ
Mimiclaw はほぼすべての Linux デバイスにデプロイできます！
...
Docker Compose を使えば、ローカルにインストールせずに Mimiclaw を実行できます。
...
> [!TIP]
> **Docker ユーザー**: デフォルトでは、Gateway は `127.0.0.1` でリッスンしており、ホストからアクセスできません。ヘルスチェックエンドポイントにアクセスしたり、ポートを公開したりする必要がある場合は、環境変数で `MIMICLAW_GATEWAY_HOST=0.0.0.0` を設定するか、`config.json` を更新してください。
...
```bash
# 質問を投げる
docker compose run --rm mimiclaw-agent -m "What is 2+2?"
...
> [!TIP]
> `~/.mimiclaw/config.json` に API キーを設定してください。
...
Telegram、Discord、QQ、DingTalk、LINE、WeCom で Mimiclaw と会話できます
...
> **Docker Compose**: `mimiclaw-gateway` サービスに `ports: ["18791:18791"]` を追加して Webhook ポートを公開してください。
...
Mimiclaw は2種類の WeCom 統合をサポートしています：
...
設定ファイル: `~/.mimiclaw/config.json`

### ワークスペース構成

Mimiclaw は設定されたワークスペース（デフォルト: `~/.mimiclaw/workspace`）にデータを保存します：
...
Mimiclaw はデフォルトでサンドボックス環境で実行されます。エージェントは設定されたワークスペース内のファイルにのみアクセスし、コマンドを実行できます。
...
| オプション | デフォルト | 説明 |
|-----------|-----------|------|
| `workspace` | `~/.mimiclaw/workspace` | エージェントの作業ディレクトリ |
...
Mimiclaw は自動的に定期タスクを実行できます。ワークスペースに `HEARTBEAT.md` ファイルを作成します：
...
**環境変数:**
- `MIMICLAW_HEARTBEAT_ENABLED=false` で無効化
- `MIMICLAW_HEARTBEAT_INTERVAL=60` で間隔変更
...
> **新機能！** Mimiclaw は現在 **モデル中心** の設定アプローチを採用しています。`ベンダー/モデル` 形式（例: `zhipu/glm-4.7`）を指定するだけで、新しいプロバイダーを追加できます—**コードの変更は一切不要！**
...
```
> OAuth認証を設定するには、`mimiclaw auth login --provider anthropic` を実行してください。

#### ロードバランシング

同じモデル名で複数のエンドポイントを設定すると、Mimiclaw が自動的にラウンドロビンで分散します：
...
| コマンド | 説明 |
|---------|------|
| `mimiclaw onboard` | 設定＆ワークスペースの初期化 |
| `mimiclaw agent -m "..."` | エージェントとチャット |
| `mimiclaw agent` | インタラクティブチャットモード |
| `mimiclaw gateway` | ゲートウェイを起動 |
| `mimiclaw status` | ステータスを表示 |
...
検索 API キーをまだ設定していない場合、これは正常です。Mimiclaw は手動検索用の便利なリンクを提供します。
...
別のインスタンスが実行中の場合に発生します。`mimiclaw gateway` が 1 つだけ実行されていることを確認してください。
