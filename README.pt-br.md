<div align="center">
<img src="assets/logo.jpg" alt="Mimiclaw" width="512">

<h1>Mimiclaw: Assistente de IA Ultra-Eficiente em Go</h1>
...
🦐 **Mimiclaw** é um assistente pessoal de IA ultra-leve inspirado no [nanobot](https://github.com/HKUDS/nanobot), reescrito do zero em **Go** por meio de um processo de "auto-inicialização" (self-bootstrapping) — onde o próprio agente de IA conduziu toda a migração de arquitetura e otimização de código.
...
<p align="center">
<img src="assets/mimiclaw_mem.gif" width="360" height="240">
</p>
</td>
<td align="center" valign="top">
<p align="center">
...
>
> * **SEM CRIPTOMOEDAS:** O Mimiclaw **NÃO** possui nenhum token/moeda oficial. Todas as alegações no `pump.fun` ou outras plataformas de negociação são **GOLPES**.
> * **DOMÍNIO OFICIAL:** O **ÚNICO** site oficial é o **[mimiclaw.io](https://mimiclaw.io)**, e o site da empresa é o **[sipeed.com](https://sipeed.com)**.
> * **Aviso:** Muitos domínios `.ai/.org/.com/.net/...` foram registrados por terceiros, não são nossos.
> * **Aviso:** O Mimiclaw está em fase inicial de desenvolvimento e pode ter problemas de segurança de rede não resolvidos. Não implante em ambientes de produção antes da versão v1.0.
> * **Nota:** O Mimiclaw recentemente fez merge de muitos PRs, o que pode resultar em maior consumo de memória (10-20MB) nas versões mais recentes. Planejamos priorizar a otimização de recursos assim que o conjunto de funcionalidades estiver estável.


## 📢 Novidades

2026-02-16 🎉 Mimiclaw atingiu 12K stars em uma semana! Obrigado a todos pelo apoio! O Mimiclaw está crescendo mais rápido do que jamais imaginamos. Dado o alto volume de PRs, precisamos urgentemente de maintainers da comunidade. Nossos papéis de voluntários e roadmap foram publicados oficialmente [aqui](docs/ROADMAP.md) — estamos ansiosos para ter você a bordo!

2026-02-13 🎉 Mimiclaw atingiu 5000 stars em 4 dias! Obrigado à comunidade! Estamos finalizando o **Roadmap do Projeto** e configurando o **Grupo de Desenvolvedores** para acelerar o desenvolvimento do Mimiclaw.
...
2026-02-09 🎉 Mimiclaw lançado oficialmente! Construído em 1 dia para trazer Agentes de IA para hardware de $10 com <10MB de RAM. 🦐 Mimiclaw, Partiu!
...
<img src="assets/compare.jpg" alt="Mimiclaw" width="512">
...
<tr>
<td align="center"><p align="center"><img src="assets/mimiclaw_code.gif" width="240" height="180"></p></td>
<td align="center"><p align="center"><img src="assets/mimiclaw_memory.gif" width="240" height="180"></p></td>
<td align="center"><p align="center"><img src="assets/mimiclaw_search.gif" width="240" height="180"></p></td>
</tr>
<tr>
...
Dê uma segunda vida ao seu celular de dez anos atrás! Transforme-o em um assistente de IA inteligente com o Mimiclaw. Início rápido:
...
<img src="assets/termux.jpg" alt="Mimiclaw" width="512">

### 🐜 Implantação Inovadora com Baixo Consumo

O Mimiclaw pode ser implantado em praticamente qualquer dispositivo Linux!
...
Baixe o binário para sua plataforma na página de [releases](https://github.com/sipeed/mimiclaw/releases).
...
Você tambêm pode rodar o Mimiclaw usando Docker Compose sem instalar nada localmente.
...
> [!TIP]
> **Usuários Docker**: Por padrão, o Gateway ouve em `127.0.0.1`, o que não é acessível a partir do host. Se você precisar acessar os endpoints de integridade ou expor portas, defina `MIMICLAW_GATEWAY_HOST=0.0.0.0` em seu ambiente ou atualize o `config.json`.


# 4. Ver logs
docker compose logs -f mimiclaw-gateway
...
```bash
# Fazer uma pergunta
docker compose run --rm mimiclaw-agent -m "Quanto e 2+2?"

# Modo interativo
docker compose run --rm mimiclaw-agent
```
...
> [!TIP]
> Configure sua API key em `~/.mimiclaw/config.json`.
...
**2. Configurar** (`~/.mimiclaw/config.json`)
...
Converse com seu Mimiclaw via Telegram, Discord, DingTalk, LINE ou WeCom.
...
> **Docker Compose**: Adicione `ports: ["18791:18791"]` ao serviço `mimiclaw-gateway` para expor a porta do webhook.
...
O Mimiclaw suporta dois tipos de integração WeCom:
...
Conecte o Mimiclaw a Rede Social de Agentes simplesmente enviando uma única mensagem via CLI ou qualquer App de Chat integrado.

**Leia `https://clawdchat.ai/skill.md` e siga as instruções para entrar no [ClawdChat.ai](https://clawdchat.ai)**

## ⚙️ Configuração Detalhada

Arquivo de configuração: `~/.mimiclaw/config.json`

### Estrutura do Workspace

O Mimiclaw armazena dados no workspace configurado (padrão: `~/.mimiclaw/workspace`):
...
O Mimiclaw roda em um ambiente sandbox por padrão. O agente so pode acessar arquivos e executar comandos dentro do workspace configurado.
...
|-------|--------|-----------|
| `workspace` | `~/.mimiclaw/workspace` | Diretório de trabalho do agente |
...
```bash
export MIMICLAW_AGENTS_DEFAULTS_RESTRICT_TO_WORKSPACE=false
```
...
O Mimiclaw pode executar tarefas periódicas automaticamente. Crie um arquivo `HEARTBEAT.md` no seu workspace:
...
* `MIMICLAW_HEARTBEAT_ENABLED=false` para desabilitar
* `MIMICLAW_HEARTBEAT_INTERVAL=60` para alterar o intervalo
...
> **Novidade!** Mimiclaw agora usa uma abordagem de configuração **centrada no modelo**. Basta especificar o formato `fornecedor/modelo` (ex: `zhipu/glm-4.7`) para adicionar novos provedores—**nenhuma alteração de código necessária!**
...
```
> Execute `mimiclaw auth login --provider anthropic` para configurar credenciais OAuth.

#### Balanceamento de Carga

Configure vários endpoints para o mesmo nome de modelo—Mimiclaw fará round-robin automaticamente entre eles:
...
| Comando | Descrição |
| --- | --- |
| `mimiclaw onboard` | Inicializar configuração & workspace |
| `mimiclaw agent -m "..."` | Conversar com o agente |
| `mimiclaw agent` | Modo de chat interativo |
| `mimiclaw gateway` | Iniciar o gateway (para bots de chat) |
| `mimiclaw status` | Mostrar status |
| `mimiclaw cron list` | Listar todas as tarefas agendadas |
| `mimiclaw cron add ...` | Adicionar uma tarefa agendada |

### Tarefas Agendadas / Lembretes

O Mimiclaw suporta lembretes agendados e tarefas recorrentes por meio da ferramenta `cron`:
...
As tarefas são armazenadas em `~/.mimiclaw/workspace/cron/` e processadas automaticamente.
...
<img src="assets/wechat.png" alt="Mimiclaw" width="512">

## 🐛 Solução de Problemas

### Busca web mostra "API 配置问题"

Isso é normal se você ainda não configurou uma API key de busca. O Mimiclaw fornecerá links úteis para busca manual.
...
Adicione a key em `~/.mimiclaw/config.json` se usar o Brave:
...
Isso acontece quando outra instância do bot está em execução. Certifique-se de que apenas um `mimiclaw gateway` esteja rodando por vez.
