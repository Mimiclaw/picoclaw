...
func skillsHelp() {
	...
	fmt.Println("  mimiclaw skills install --registry clawhub github")
}
...
func skillsInstallCmd(installer *skills.SkillInstaller, cfg *config.Config) {
	if len(os.Args) < 4 {
		fmt.Println("Usage: mimiclaw skills install <github-repo>")
		fmt.Println("       mimiclaw skills install --registry <name> <slug>")
		return
	}
	...
	if os.Args[3] == "--registry" {
		if len(os.Args) < 6 {
			fmt.Println("Usage: mimiclaw skills install --registry <name> <slug>")
			fmt.Println("Example: mimiclaw skills install --registry clawhub github")
			return
		}
...
func skillsInstallBuiltinCmd(workspace string) {
	builtinSkillsDir := "./mimiclaw/skills"
...
func skillsListBuiltinCmd() {
	...
	builtinSkillsDir := filepath.Join(filepath.Dir(cfg.WorkspacePath()), "mimiclaw", "skills")
