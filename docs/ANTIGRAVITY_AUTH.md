...
**Antigravity** (Google Cloud Code Assist) is a Google-backed AI model provider that offers access to models like Claude Opus 4.6 and Gemini through Google's Cloud infrastructure. This document provides a complete guide on how authentication works, how to fetch models, and how to implement a new provider in Mimiclaw.
...
10. [Creating a New Provider in Mimiclaw](#creating-a-new-provider-in-mimiclaw)
...
const antigravityPlugin = {
  ...
  register(api: MimiclawPluginApi) {
...
type ProviderAuthResult = {
  ...
  configPatch?: Partial<MimiclawConfig>;
  defaultModel?: string;
  notes?: string[];
};
```
...
- Go ≥ 1.21
- Mimiclaw codebase (`pkg/providers/` and `pkg/auth/`)
...
Auth profiles are stored in `~/.mimiclaw/auth.json`:
...
## Creating a New Provider in Mimiclaw

Mimiclaw providers are implemented as Go packages under `pkg/providers/`. To add a new provider:
...
If your provider requires OAuth or special authentication, add a case to `cmd/mimiclaw/cmd_auth.go`:
...
mimiclaw auth login --provider your-provider

# List models (for Antigravity)
mimiclaw auth models

# Start the gateway
mimiclaw gateway

# Run an agent with a specific model
mimiclaw agent -m "Hello" --model your-model
```
...
export MIMICLAW_AGENTS_DEFAULTS_MODEL=your-model

# Override provider settings
export MIMICLAW_MODEL_LIST='[{"model_name":"your-model","model":"your-provider/model-name","api_key":"..."}]'
```
...
### "Token expired"
- Refresh OAuth tokens: `mimiclaw auth login --provider antigravity`
...
- Check auth profile storage: `~/.mimiclaw/auth.json`
- Re-run `mimiclaw auth login --provider antigravity`
