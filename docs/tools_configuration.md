...
Mimiclaw's tools configuration is located in the `tools` field of `config.json`.
...
By default, Mimiclaw blocks the following dangerous commands:
...
All configuration options can be overridden via environment variables with the format `MIMICLAW_TOOLS_<SECTION>_<KEY>`:

For example:
- `MIMICLAW_TOOLS_WEB_BRAVE_ENABLED=true`
- `MIMICLAW_TOOLS_EXEC_ENABLE_DENY_PATTERNS=false`
- `MIMICLAW_TOOLS_CRON_EXEC_TIMEOUT_MINUTES=10`
