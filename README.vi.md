...
<h1>Mimiclaw: Trợ lý AI Siêu Nhẹ viết bằng Go</h1>
...
2026-02-16 🎉 Mimiclaw đạt 12K stars chỉ trong một tuần! Cảm ơn tất cả mọi người! Mimiclaw đang phát triển nhanh hơn chúng tôi tưởng tượng. Do số lượng PR tăng cao, chúng tôi cấp thiết cần maintainer từ cộng đồng. Các vai trò tình nguyện viên và roadmap đã được công bố [tại đây](docs/ROADMAP.md) — rất mong đón nhận sự tham gia của bạn!

2026-02-13 🎉 Mimiclaw đạt 5000 stars trong 4 ngày! Cảm ơn cộng đồng! Chúng tôi đang hoàn thiện **Lộ trình dự án (Roadmap)** và thiết lập **Nhóm phát triển** để đẩy nhanh tốc độ phát triển Mimiclaw.  
...
2026-02-09 🎉 Mimiclaw chính thức ra mắt! Được xây dựng trong 1 ngày để mang AI Agent đến phần cứng $10 với RAM <10MB. 🦐 Mimiclaw, Lên Đường!
...
Mimiclaw có thể triển khai trên hầu hết mọi thiết bị Linux!
...
Bạn cũng có thể chạy Mimiclaw bằng Docker Compose mà không cần cài đặt gì trên máy.
...
> [!TIP]
> **Người dùng Docker**: Theo mặc định, Gateway lắng nghe trên `127.0.0.1`, không thể truy cập từ máy chủ. Nếu bạn cần truy cập các endpoint kiểm tra sức khỏe hoặc mở cổng, hãy đặt `MIMICLAW_GATEWAY_HOST=0.0.0.0` trong môi trường của bạn hoặc cập nhật `config.json`.
...
```bash
# Đặt câu hỏi
docker compose run --rm mimiclaw-agent -m "2+2 bằng mấy?"
...
> [!TIP]
> Thiết lập API key trong `~/.mimiclaw/config.json`.
...
**2. Cấu hình** (`~/.mimiclaw/config.json`)
...
```bash
mimiclaw agent -m "Xin chào, bạn là ai?"
```
...
Trò chuyện với Mimiclaw qua Telegram, Discord, DingTalk, LINE hoặc WeCom.
...
> **Docker Compose**: Thêm `ports: ["18791:18791"]` vào service `mimiclaw-gateway` để mở port webhook.
...
Mimiclaw hỗ trợ hai loại tích hợp WeCom:
...
Kết nối Mimiclaw với Mạng xã hội Agent chỉ bằng cách gửi một tin nhắn qua CLI hoặc bất kỳ ứng dụng Chat nào đã tích hợp.

**Đọc `https://clawdchat.ai/skill.md` và làm theo hướng dẫn để tham gia [ClawdChat.ai](https://clawdchat.ai)**

## ⚙️ Cấu hình chi tiết

File cấu hình: `~/.mimiclaw/config.json`

### Cấu trúc Workspace

Mimiclaw lưu trữ dữ liệu trong workspace đã cấu hình (mặc định: `~/.mimiclaw/workspace`):
...
Mimiclaw chạy trong môi trường sandbox theo mặc định. Agent chỉ có thể truy cập file và thực thi lệnh trong phạm vi workspace.
...
|----------|---------|-------|
| `workspace` | `~/.mimiclaw/workspace` | Thư mục làm việc của agent |
...
Mimiclaw có thể tự động thực hiện các tác vụ định kỳ. Tạo file `HEARTBEAT.md` trong workspace:
...
* `MIMICLAW_HEARTBEAT_ENABLED=false` để tắt
* `MIMICLAW_HEARTBEAT_INTERVAL=60` để thay đổi khoảng thời gian
...
> **Tính năng mới!** Mimiclaw hiện sử dụng phương pháp cấu hình **đặt mô hình vào trung tâm**. Chỉ cần chỉ định dạng `nhà cung cấp/mô hình` (ví dụ: `zhipu/glm-4.7`) để thêm nhà cung cấp mới—**không cần thay đổi mã!**
...
```
> Chạy `mimiclaw auth login --provider anthropic` để thiết lập thông tin xác thực OAuth.

#### Cân bằng Tải tải

Định cấu hình nhiều endpoint cho cùng một tên mô hình—Mimiclaw sẽ tự động phân phối round-robin giữa chúng:
...
| Lệnh | Mô tả |
| --- | --- |
| `mimiclaw onboard` | Khởi tạo cấu hình & workspace |
| `mimiclaw agent -m "..."` | Trò chuyện với agent |
| `mimiclaw agent` | Chế độ chat tương tác |
| `mimiclaw gateway` | Khởi động gateway (cho bot chat) |
| `mimiclaw status` | Hiển thị trạng thái |
| `mimiclaw cron list` | Liệt kê tất cả tác vụ định kỳ |
| `mimiclaw cron add ...` | Thêm tác vụ định kỳ |

### Tác vụ định kỳ / Nhắc nhở

Mimiclaw hỗ trợ nhắc nhở theo lịch và tác vụ lặp lại thông qua công cụ `cron`:
...
Các tác vụ được lưu trong `~/.mimiclaw/workspace/cron/` và được xử lý tự động.
...
Điều này là bình thường nếu bạn chưa cấu hình API key cho tìm kiếm. Mimiclaw sẽ cung cấp các liên kết hữu ích để tìm kiếm thủ công.
...
Thêm key vào `~/.mimiclaw/config.json` nếu dùng Brave:
...
Điều này xảy ra khi có một instance bot khác đang chạy. Đảm bảo chỉ có một tiến trình `mimiclaw gateway` chạy tại một thời điểm.
