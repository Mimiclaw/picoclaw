<div align="center">
  ...
  <h1>Mimiclaw : Assistant IA Ultra-Efficace en Go</h1>
...
2026-02-16 🎉 Mimiclaw a atteint 12K étoiles en une semaine ! Merci à tous pour votre soutien ! Mimiclaw grandit plus vite que nous ne l'avions jamais imaginé. Vu le volume élevé de PR, nous avons un besoin urgent de mainteneurs communautaires. Nos rôles de bénévoles et notre feuille de route sont officiellement publiés [ici](docs/ROADMAP.md) — nous avons hâte de vous accueillir !

2026-02-13 🎉 Mimiclaw a atteint 5000 étoiles en 4 jours ! Merci à la communauté ! Nous finalisons la **Feuille de Route du Projet** et mettons en place le **Groupe de Développeurs** pour accélérer le développement de Mimiclaw.
...
2026-02-09 🎉 Mimiclaw est lancé ! Construit en 1 jour pour apporter les Agents IA au matériel à 10$ avec <10 Mo de RAM. 🦐 Mimiclaw, c'est parti !
...
Donnez une seconde vie à votre téléphone d'il y a dix ans ! Transformez-le en assistant IA intelligent avec Mimiclaw. Démarrage rapide :
...
Mimiclaw peut être déployé sur pratiquement n'importe quel appareil Linux !
...
Vous pouvez également exécuter Mimiclaw avec Docker Compose sans rien installer localement.
...
> [!TIP]
> **Utilisateurs Docker** : Par défaut, le Gateway écoute sur `127.0.0.1`, ce qui n'est pas accessible depuis l'hôte. Si vous avez besoin d'accéder aux endpoints de santé ou d'exposer des ports, définissez `MIMICLAW_GATEWAY_HOST=0.0.0.0` dans votre environnement ou mettez à jour `config.json`.
...
```bash
# Poser une question
docker compose run --rm mimiclaw-agent -m "Combien font 2+2 ?"
...
> [!TIP]
> Configurez votre clé API dans `~/.mimiclaw/config.json`.
...
**2. Configurer** (`~/.mimiclaw/config.json`)
...
```bash
mimiclaw agent -m "Combien font 2+2 ?"
```
...
Discutez avec votre Mimiclaw via Telegram, Discord, DingTalk, LINE ou WeCom
...
> **Docker Compose** : Ajoutez `ports: ["18791:18791"]` au service `mimiclaw-gateway` pour exposer le port du webhook.
...
Mimiclaw prend en charge deux types d'intégration WeCom :
...
Connectez Mimiclaw au Réseau Social d'Agents simplement en envoyant un seul message via le CLI ou n'importe quelle application de chat intégrée.

**Lisez `https://clawdchat.ai/skill.md` et suivez les instructions pour rejoindre [ClawdChat.ai](https://clawdchat.ai)**

## ⚙️ Configuration

Fichier de configuration : `~/.mimiclaw/config.json`

### Structure du Workspace

Mimiclaw stocke les données dans votre workspace configuré (par défaut : `~/.mimiclaw/workspace`) :
...
Mimiclaw s'exécute dans un environnement sandboxé par défaut. L'agent ne peut accéder aux fichiers et exécuter des commandes qu'au sein du workspace configuré.
...
|--------|------------|-------------|
| `workspace` | `~/.mimiclaw/workspace` | Répertoire de travail de l'agent |
...
Mimiclaw peut exécuter des tâches périodiques automatiquement. Créez un fichier `HEARTBEAT.md` dans votre workspace :
...
* `MIMICLAW_HEARTBEAT_ENABLED=false` pour désactiver
* `MIMICLAW_HEARTBEAT_INTERVAL=60` pour modifier l'intervalle
...
```bash
mimiclaw agent -m "Bonjour, comment ça va ?"
```
...
> **Nouveau !** Mimiclaw utilise désormais une approche de configuration **centrée sur le modèle**. Spécifiez simplement le format `fournisseur/modèle` (par exemple, `zhipu/glm-4.7`) pour ajouter de nouveaux fournisseurs—**aucune modification de code requise !**
...
```
> Exécutez `mimiclaw auth login --provider anthropic` pour configurer les identifiants OAuth.

#### Équilibrage de Charge

Configurez plusieurs points de terminaison pour le même nom de modèle—Mimiclaw utilisera automatiquement le round-robin entre eux :
...
| ------------------------- | ------------------------------------- |
| `mimiclaw onboard`        | Initialiser la configuration & le workspace |
| `mimiclaw agent -m "..."` | Discuter avec l'agent                 |
| `mimiclaw agent`          | Mode de discussion interactif         |
| `mimiclaw gateway`        | Démarrer la passerelle                |
| `mimiclaw status`         | Afficher le statut                    |
| `mimiclaw cron list`      | Lister toutes les tâches planifiées   |
| `mimiclaw cron add ...`   | Ajouter une tâche planifiée           |

### Tâches Planifiées / Rappels

Mimiclaw prend en charge les rappels planifiés et les tâches récurrentes via l'outil `cron` :
...
Les tâches sont stockées dans `~/.mimiclaw/workspace/cron/` et traitées automatiquement.
...
C'est normal si vous n'avez pas encore configuré de clé API de recherche. Mimiclaw fournira des liens utiles pour la recherche manuelle.
...
Ajoutez la clé dans `~/.mimiclaw/config.json` si vous utilisez Brave :
...
Cela se produit lorsqu'une autre instance du bot est en cours d'exécution. Assurez-vous qu'un seul `mimiclaw gateway` fonctionne à la fois.
