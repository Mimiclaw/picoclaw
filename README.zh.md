...
<h1>Mimiclaw: 基于Go语言的超高效 AI 助手</h1>
...
2026-02-16 🎉 Mimiclaw 在一周内突破了12K star! 感谢大家的关注！Mimiclaw 的成长速度超乎我们预期. 由于PR数量的快速膨胀，我们亟需社区开发者参与维护. 我们需要的志愿者角色和roadmap已经发布到了[这里](docs/ROADMAP.md), 期待你的参与！

2026-02-13 🎉 **Mimiclaw 在 4 天内突破 5000 Stars！** 感谢社区的支持！由于正值中国春节假期，PR 和 Issue 涌入较多，我们正在利用这段时间敲定 **项目路线图 (Roadmap)** 并组建 **开发者群组**，以便加速 Mimiclaw 的开发。
🚀 **行动号召：** 请在 GitHub Discussions 中提交您的功能请求 (Feature Requests)。我们将在接下来的周会上进行审查和优先级排序。

2026-02-09 🎉 **Mimiclaw 正式发布！** 仅用 1 天构建，旨在将 AI Agent 带入 10 美元硬件与 <10MB 内存的世界。🦐 Mimiclaw（皮皮虾），我们走！
...
mimiclaw 可以将你10年前的老旧手机废物利用，变身成为你的AI助理！快速指南:
...
然后跟随下面的“快速开始”章节继续配置mimiclaw即可使用！  
<img src="assets/termux.jpg" alt="Mimiclaw" width="512">

### 🐜 创新的低占用部署

Mimiclaw 几乎可以部署在任何 Linux 设备上！
...
您也可以使用 Docker Compose 运行 Mimiclaw，无需在本地安装任何环境。
...
> [!TIP]
**Docker 用户**: 默认情况下, Gateway监听 `127.0.0.1`，这使得这个端口未暴露到容器外。如果你需要通过端口映射访问健康检查接口, 请在环境变量中设置 `MIMICLAW_GATEWAY_HOST=0.0.0.0` 或修改 `config.json`。
...
```bash
# 提问
docker compose run --rm mimiclaw-agent -m "2+2 等于几？"
...
> [!TIP]
> 在 `~/.mimiclaw/config.json` 中设置您的 API Key。
...
**2. 配置 (Configure)** (`~/.mimiclaw/config.json`)
...
Mimiclaw 支持多种聊天平台，使您的 Agent 能够连接到任何地方。
...
只需通过 CLI 或任何集成的聊天应用发送一条消息，即可将 Mimiclaw 连接到 Agent 社交网络。

\*\*阅读 `https://clawdchat.ai/skill.md` 并按照说明加入 [ClawdChat.ai](https://clawdchat.ai)

## ⚙️ 配置详解

配置文件路径: `~/.mimiclaw/config.json`

### 工作区布局 (Workspace Layout)

Mimiclaw 将数据存储在您配置的工作区中（默认：`~/.mimiclaw/workspace`）：
...
Mimiclaw 可以自动执行周期性任务。在工作区创建 `HEARTBEAT.md` 文件：
...
- `MIMICLAW_HEARTBEAT_ENABLED=false` 禁用
- `MIMICLAW_HEARTBEAT_INTERVAL=60` 更改间隔
...
> **新功能！** Mimiclaw 现在采用**以模型为中心**的配置方式。只需使用 `厂商/模型` 格式（如 `zhipu/glm-4.7`）即可添加新的 provider——**无需修改任何代码！**
...
> 运行 `mimiclaw auth login --provider anthropic` 来设置 OAuth 凭证。
...
为同一个模型名称配置多个端点——Mimiclaw 会自动在它们之间轮询：
...
| 命令                      | 描述               |
| ------------------------- | ------------------ |
| `mimiclaw onboard`        | 初始化配置和工作区 |
| `mimiclaw agent -m "..."` | 与 Agent 对话      |
| `mimiclaw agent`          | 交互式聊天模式     |
| `mimiclaw gateway`        | 启动网关 (Gateway) |
| `mimiclaw status`         | 显示状态           |
| `mimiclaw cron list`      | 列出所有定时任务   |
| `mimiclaw cron add ...`   | 添加定时任务       |

### 定时任务 / 提醒 (Scheduled Tasks)

Mimiclaw 通过 `cron` 工具支持定时提醒和重复任务：
...
任务存储在 `~/.mimiclaw/workspace/cron/` 中并自动处理。
...
如果您尚未配置搜索 API Key，这是正常的。Mimiclaw 会提供手动搜索的帮助链接。
...
这表示有另一个机器人实例正在运行。请确保同一时间只有一个 `mimiclaw gateway` 进程在运行。
